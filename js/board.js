...
(function () {
  ...
  // 실제 사용자 게시글처럼 보이는 샘플 데이터는 js/sample-posts.js로 분리했다 --
  // index.html의 메인 카드(MOMENTRIP PICK/HOT/주변 인기 장소/지역 행사/커뮤니티
  // 미리보기)도 board-list/board-view와 완전히 동일한 데이터를 가리켜야 해서,
  // 이 배열을 여기 사본으로 다시 두지 않고 window.MomentripSamplePosts 하나만
  // 공유한다(각 HTML이 js/sample-posts.js를 이 파일보다 먼저 로드). id는 CRUD의
  // `post-` 접두사와 겹치지 않도록 `sample-` 접두사를 쓴다(js/sample-posts.js 참고).
  var SAMPLE_POSTS = window.MomentripSamplePosts || [];
